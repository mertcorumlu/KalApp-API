...
class Auth
{
    ...
    public function login_okulno($okulno, $password, $remember = 0,$token, $captcha = NULL)
    {
        $return['error'] = true;
        ...
        if (strlen($okulno) == 0) {
            $this->addAttempt();
            $return['message'] = "Okul Numarası hatalı";
        ...
        $uid = $this->getUID_okulno(strtolower($okulno));
        ...
        $sessiondata = $this->addSession_cms($user['uid'], $remember,$token);
    ...
    */
    ...
    public function getUID_okulno($okulno)
    {
        $query = $this->dbh->prepare("SELECT id FROM {$this->config->table_users} WHERE okul_no = ?");
        $query->execute(array($okulno));
    ...
    */
    ...
    protected function addSession_cms($uid, $remember, $token)
    {
        $ip = $this->getIp();
        $user = $this->getBaseUser($uid);
        ...
        $query = $this->dbh->prepare("INSERT INTO {$this->config->table_sessions} (uid, hash, fcms_token, expiredate, ip, agent, cookie_crc) VALUES (?, ?, ?, ?, ?, ?, ?)");

        if (!$query->execute(array($uid, $data['hash'],$token, $data['expire'], $ip, $agent, $data['cookie_crc']))) {
            return false;
        }
    ...
    */
    ...
    public function addUser($email, $password, $params = array(), &$sendmail)
    {
        $return['error'] = true;
        ...
        if (!$query->execute()) {
            $return['message'] =$query->errorInfo()[2];
            return $return;
        }
